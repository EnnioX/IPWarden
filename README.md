...
IPWarden是一个IP资产风险发现工具，对目标IP循环扫描更新结果，特点在于发现目标IP范围的Web资产与管理后台并自动调用xray扫描，将扫描产生的数据结果通过API返回，方便调用加工。适合甲方安全人员管理公网/内网资产安全风险暴露面，渗透测试人员用于信息收集和攻击面探测。集成的工具有:nmap、masscan、TideFinger、xray
