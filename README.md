...
IPWarden是一个IP资产风险监控工具，可对大量IP/网段循环扫描。在主机、端口&风险端口、协议发现的功能基础上，探测Web资产并对Web资产进行组件指纹扫描、CMS识别、管理后台识别、ssl证书收集、xray漏洞扫描。本工具特点为所有扫描结果均可通过API返回json数据和导出为xlsx表格，方便二次开发与数据加工。适合甲方安全人员管理公网/内网IP资产风险暴露面，渗透测试人员用于批量信息收集和攻击面探测。集成的工具有:nmap、masscan、TideFinger、xray

后续更新计划：集成对风险端口的自动化安全测试
