...
IPWarden（守望者）是一个IP资产风险巡查工具。持续发现系统、Web两个维度的资产信息、安全风险和漏洞。所有扫描结果可通过API请求返回json数据和导出xlsx，方便二次开发与数据加工。适合甲方安全人员管理公网/内网IP资产风险暴露面。
...
9. SSL证书信息扫描
10. 首页汇总数据生成统计图
11. 数据汇总xlsx文件导出（xray扫描信息只导出基本字段信息到表格，更多漏洞详情与请求响应包需通过API查询）
