...
IPWarden是一个IP资产风险发现工具，可循环扫描IP资产，更新风险暴露数据。在主机、端口&协议发现与风险端口管理的基础上，探测Web资产并对Web资产进行组件指纹扫描、CMS识别、管理后台识别、ssl证书收集、xray漏洞扫描。本工具特点为所有扫描结果均可通过API返回json数据和导出为xlsx表格，方便二次开发与数据加工。适合甲方安全人员管理公网/内网IP资产风险暴露面。集成的工具有:nmap、masscan、TideFinger、xray
