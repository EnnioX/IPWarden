...
IPWarden是一个IP资产风险监控工具，可对大量IP/网段循环扫描。有主机、端口、协议发现的基本功能，另外可探测Web资产与管理后台并进行组件指纹扫描和漏洞扫描。所有扫描结果均可通过API返回json数据和导出为xlsx表格，方便数据二次加工。适合甲方安全人员管理公网/内网IP资产风险暴露面，渗透测试人员用于信息收集和攻击面探测。集成的工具有:nmap、masscan、TideFinger、xray
